...
- Allow configuration comparison between two backups or two watches (see FR-007a).
- Provide a plugin architecture for additional Garmin watches, scoped to Phase 4 (see Section 8.4).
...
The `timezone` field defaults to `auto_gps` (leave the watch's satellite-derived timezone behavior untouched) unless the user explicitly sets a fixed zone identifier. The tool MUST NOT silently override auto-detected timezone behavior.
...
### FR-007a — Bundle-to-bundle comparison

The application MUST support comparing two saved bundles, or a saved bundle against a second connected device, without requiring either side to be the currently connected watch. This comparison:

- MUST classify each setting using the same categories as FR-007.
- MUST NOT require write access to either source.
- MUST flag settings that are model- or firmware-specific to one side only.
- SHOULD be available from both the GUI (FR-015) and CLI (FR-016).

This satisfies the Section 4.2 secondary goal of comparing two backups or two watches, and is distinct from FR-007's live-device diff.
...
gwo compare <bundle-a> <bundle-or-device-b>
...
### FR-021 — Repository documentation compliance

The repository MUST adopt and maintain the Software Project Living Documentation Standard 2.2, per Section 24. Specifically:

- A resolvable authority MUST exist for every responsibility Section 24's tier declaration requires, declared in `PROJECT-STANDARD.md`'s Authority Mapping and mirrored in `.docs-authority.json`.
- `scripts/docs-check.ps1` (or an equivalent) MUST be present and runnable, and its compliance table MUST be regenerated into `ASSESSMENT.md` at each merge to main.
- Every Class 2+ change (Section 24) MUST follow the routed-read and documentation-update rules before being considered complete, including changes made by an AI coding agent.
- This project remains Tier 2, not Tier 2C: `REQUIREMENTS.md` (this document) is not a frozen contractual authority. It continues to be edited directly as scope evolves, subject to the same review discipline as any other living document.

### 7.1 Risk Classification

FR-002, FR-007, FR-008, and Section 9 reference a per-setting "risk classification." Every capability MUST be assigned one of the following levels, and the assignment MUST be documented in the capability manifest alongside its supporting evidence (Section 19):

| Level | Definition | Examples | Confirmation required |
| --- | --- | --- | --- |
| **Low** | Reversible through the declarative layer alone; no restart or sync needed; no data loss possible. | Activity favorites, display units, time format. | Standard plan approval. |
| **Medium** | Reversible, but requires a restart, sync, or re-application to fully undo; no data loss. | Data screen layout, controls/glances order, power mode. | Standard plan approval; surfaced distinctly in the plan. |
| **High** | Reversible only through native Garmin backup/restore or guided manual steps; partial state possible if interrupted. | Watch face selection, Connect IQ properties, sensor pairing preferences. | Explicit per-item confirmation; `require_confirmation_for_high_risk` MUST default to true. |
| **Destructive** | Not reliably reversible, affects data outside declarative scope, or has no known recovery mechanism. | Anything touching activity history, health/wellness records, or a factory reset. | Out of scope per Section 5; MUST be blocked, not merely confirmed. |

An adapter MUST NOT apply a High or Destructive setting without the corresponding recovery coverage described in Section 9. Unclassified settings MUST default to High until evidence supports a lower classification.
...
### 8.4 Plugin architecture boundary (Phase 4)

The Section 4.2 goal of a "plugin architecture for additional Garmin watches" is explicitly scoped to Phase 4 and is distinct from the internal adapter interfaces defined in 8.1. Internal adapters ship with the application and are trusted by default; a plugin is third-party code loaded at runtime and MUST NOT be trusted by default.

Before any plugin loading is enabled, the project MUST define, as a prerequisite Phase 4 deliverable:

- A plugin manifest format declaring supported models, firmware ranges, required adapter interfaces, and permissions requested.
- A capability-declaration contract so a plugin cannot claim support it has not demonstrated.
- A sandboxing or process-isolation boundary between plugin code and the core application, including filesystem and device-access limits.
- A signing or verification mechanism before a plugin is loaded outside of explicit developer mode.

Until this contract exists, "additional watch support" MUST be implemented as first-party adapters and capability profiles under Section 8.1, not as external plugins. This document does not authorize third-party plugin loading before that contract is written and reviewed.
...
- Line coverage MUST remain at or above 85% for the domain/planning layers (schema, diff, plan, journal, redaction); platform-automation and adapter modules are exempt from the numeric threshold but MUST retain contract-test coverage per adapter. CI MUST fail below this threshold.
...
- Scaffold the Living Documentation Standard 2.2 authority mapping (`PROJECT-STANDARD.md`, `.docs-authority.json`, `AGENTS.md`) and required Tier 2 authorities, per Section 24, before feature work begins.
...
- Additional Garmin watch models, initially as first-party adapters.
- Plugin manifest, capability-declaration, sandboxing, and signing contract per Section 8.4, required before any third-party plugin loading is enabled.
...
"Codex" in this document refers generically to whichever coding agent or engineer is implementing this specification — it is not a reference to a specific product, and this document should be handed to any implementing agent (human or AI) unchanged.

> **Authority note (Section 24):** Under the Living Documentation Standard, "agent rules" is a single-valued responsibility whose canonical authority is `AGENTS.md`. The numbered instructions below are the seed content for that file. Once `AGENTS.md` exists (Section 24, Greenfield sequence), it is the authoritative agent instruction set and absorbs these points; this section becomes historical context for why they were chosen rather than a second live authority. Do not maintain both as independently current.
...
## 24. Living Documentation Standard Adoption
...
Required at Tier 2. `PROJECT-STANDARD.md` and `.docs-authority.json` are the canonical record; this table is the initial declaration to scaffold from during Phase 0.

| Responsibility | Authority | Notes |
| --- | --- | --- |
| Project overview | `README.md` | |
| Current assessment | `ASSESSMENT.md` | Includes the generated `docs-check` compliance table. |
| Architecture | `ARCHITECTURE.md` | Absorbs `docs/COMPATIBILITY.md`, `docs/ADAPTERS.md`, and `docs/ENDURO2-RESEARCH.md` from Section 19 as supporting evidence, not competing authorities. |
| Change history | `CHANGELOG.md` | Date-stamped sections; this project does not currently cut versioned releases. |
| Defect tracking | `ISSUES.md` | |
| Technical debt | `TECH-DEBT.md` | |
| Deferred improvements | `FUTURE-UPGRADES.md` | |
| Validation | `VALIDATION.md` | Absorbs `docs/SECURITY.md`'s test-facing content (secret redaction, malicious-import tests); the threat model itself lives under Architecture. |
| Operations | `OPERATIONS.md` | Required at Tier 2; covers install/runtime recovery, not just the CLI job-recovery flows already specified in FR-008/FR-009. |
| Decision history | `docs/decisions/` | ADRs for Class 4 changes per FR-021; ADR-001 should record the choice of adapter priority order in Section 2. |
...
This document (`REQUIREMENTS.md`) remains the working statement of what the product should do, but as a living document under Tier 2 — not the frozen contractual artifact the standard describes for Tier 2C. When repository behavior and this document disagree about what the system *does*, the repository governs and this document is corrected. When they disagree about what the system *should* do, this document governs until it is itself revised.

### 24.3 Change classes for this repository

Section 24's Change Classes 0-4 apply as defined in the standard, unmodified. In practice for this codebase:

- Adapter implementations, UI automation selectors, and MTP/USB file handling are Class 3 (Functional) at minimum.
- Anything touching the trust boundary between the declarative config layer and a live write to the watch — new adapter families, changes to the risk classification in Section 7.1, or changes to what counts as a "safe" apply — is Class 4 (Architectural/Security) and presumes an ADR.
- Documentation-only changes to this file are Class 1.

### 24.4 Bootstrap

Implementation agents should apply the standard's Canonical Bootstrap Instruction against this repository, using the Tier 2 declaration and Authority Mapping above rather than re-deriving them, and treating Phase 0 (Section 18) as the Greenfield sequence's starting point.
