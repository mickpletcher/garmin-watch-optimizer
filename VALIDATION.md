...
- Repository governance: `main` requires pull requests and all eight checks with administrator enforcement, linear history, resolved conversations, blocked force pushes and deletion, and squash-only merges. Default GitHub Actions workflow permissions are read only. Secret scanning, push protection, Dependabot alerts, and Dependabot security updates are enabled.
